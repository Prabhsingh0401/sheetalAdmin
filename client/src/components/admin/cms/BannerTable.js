...
import { Edit3, Trash2, ArrowUpDown, Search, RefreshCw, ChevronLeft, ChevronRight, ImageIcon, Smartphone, Monitor, Layout } from "lucide-react";
    ...
    const filteredData = useMemo(() => {
        return (Array.isArray(banners) ? banners : [])
            .filter((b) => {
                ...
                const deviceMatch = deviceFilter === "All" || b.deviceType === deviceFilter;
            ...
            <div className="overflow-x-auto min-h-[300px]">
                <table className="w-full text-sm text-left">
                    <thead className="bg-slate-50 text-slate-900 font-bold border-b border-slate-200 uppercase text-[11px] tracking-wider">
                        <tr>
                            <th className="px-4 py-4 w-12 text-center cursor-pointer" onClick={() => handleSort("order")}>
                                <div className="flex items-center justify-center gap-1">Order <ArrowUpDown size={12} /></div>
                            </th>
                            <th className="px-4 py-4 w-20 text-center">Image</th>
                            <th className="px-4 py-4 cursor-pointer group" onClick={() => handleSort("title")}>
                                <div className="flex items-center gap-1">Banner Title <ArrowUpDown size={14} className="opacity-50 group-hover:opacity-100" /></div>
                            </th>
                                ...
                                <tr key={b._id} className="hover:bg-slate-50/80 transition-colors">
                                    ...
                                    <td className="px-4 py-4">
                                        <div className="w-16 h-10 mx-auto rounded-lg bg-slate-100 border border-slate-200 overflow-hidden">
                                            {b.image?.url ? (
                                                <img
                                                    src={`${IMAGE_BASE_URL}/${b.image.url.replace(/\\/g, '/')}`.replace(/([^:]\/)\/+/g, "$1")}
                                                    alt=""
                                    ...
                                    <td className="px-4 py-4 font-bold text-slate-900">
                                        <div className="flex flex-col">
                                            <span>{b.title}</span>
                                            <span className="text-[10px] text-slate-400 font-normal truncate max-w-[200px]">{b.link}</span>
                                        </div>
                                    </td>

                                    <td className="px-4 py-4 text-slate-600 font-medium">
                                        <div className="flex items-center gap-1.5">
                                            {b.deviceType === "Mobile" ? <Smartphone size={14} /> : <Monitor size={14} />}
                                            <span className="text-xs">{b.deviceType}</span>
                                        </div>
                                    </td>

                                    <td className="px-4 py-4">
                                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${b.status === "Active" ? "bg-emerald-100 text-emerald-800" : "bg-slate-100 text-slate-600"
                                            }`}>
                                            {b.status}
                                        </span>
                                    </td>
