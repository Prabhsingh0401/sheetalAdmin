...
import { X, Truck, Loader2, CheckCircle, AlertCircle, Package } from "lucide-react";
import { updateOrderStatus } from "@/services/orderService";
import toast from "react-hot-toast";

export default function OrderModal({ isOpen, onClose, onSuccess, initialData }) {
    ...
    const handleUpdate = async () => {
        ...
        if (status === "Shipped") {
            if (!shippingInfo.carrier.trim() || !shippingInfo.trackingId.trim()) {
                return toast.error("Courier Name and Tracking ID are required for shipping.");
            }
        }
        ...
        try {
            const payload = {
                ...
                shippingInfo: (status === "Shipped" || status === "Delivered") ? shippingInfo : null,
                shippedAt: status === "Shipped" ? new Date() : initialData?.shippedAt,
                deliveredAt: status === "Delivered" ? new Date() : initialData?.deliveredAt
            };
            ...
            <div className="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in slide-in-from-bottom-4 duration-300">
                ...
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/80">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="font-black text-slate-800 tracking-tight leading-none">Manage Order</h2>
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1.5">ID: #{initialData?._id?.slice(-10)}</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="p-2.5 hover:bg-rose-50 rounded-full text-slate-400 hover:text-rose-500 transition-all border border-slate-200">
                        <X size={18} />
                    </button>
                </div>
                ...
                <div className="p-7 space-y-6">
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Process Step</label>
                    ...
                    {status === "Shipped" ? (
                        <div className="p-5 bg-blue-50/50 rounded-[2rem] border-2 border-blue-100 space-y-4 animate-in slide-in-from-top-2 duration-300">
                            <div className="flex items-center gap-2 text-blue-700 mb-1">
                                <Truck size={18} />
                                <span className="text-[10px] font-black uppercase tracking-widest">Logistics Details</span>
                            </div>
                        ...
                        <div className="p-5 bg-emerald-50 rounded-2xl border-2 border-emerald-100 flex items-start gap-3 text-emerald-700 animate-in fade-in slide-in-from-top-2">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] opacity-80 mt-1 font-medium italic underline underline-offset-2">Customer will receive an automated delivery confirmation email.</p>
                            </div>
                        </div>
                    ) : status === "Cancelled" ? (
                        <div className="p-5 bg-rose-50 rounded-2xl border-2 border-rose-100 flex items-start gap-3 text-rose-700 animate-in fade-in">
                            ...
                            <div>
                                <p className="text-xs font-bold text-rose-800">Warning: Cancellation</p>
                                <p className="text-[10px] opacity-80 mt-1">This action cannot be undone. Refund will be initiated if paid online.</p>
                            </div>
                        </div>
                    ) : null}
                </div>
