...
export default function CreateOrderModal({ isOpen, onClose, onSuccess }) {
  ...
  const addItem = (product) => {
    ...
    if (exist) {
      setSelectedItems(
        selectedItems.map((x) =>
          x.product === product._id ? { ...x, quantity: x.quantity + 1 } : x,
        ),
      );
    } else {
      setSelectedItems([
  ...
  const removeItem = (id) =>
    setSelectedItems(selectedItems.filter((x) => x.product !== id));
            ...
            <div className="grid grid-cols-1 gap-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
              {products
                ...
                .map((p) => (
                  <div
                    key={p._id}
                    className="flex items-center justify-between p-4 border border-slate-50 rounded-2xl hover:bg-slate-50/50 transition-all group"
                  >
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <button
                        type="button"
                        onClick={() => addItem(p)}
                        className="p-2 bg-white border border-slate-200 text-slate-800 rounded-lg hover:bg-slate-900 hover:text-white transition-all shadow-sm"
                      >
                        <Plus size={18} />
                      </button>
                    </div>
