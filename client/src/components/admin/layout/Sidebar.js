...
export default function Sidebar({ storeName = "Admin", isOpen, setIsOpen }) {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#111827] border-r border-white/5 text-slate-300 transform transition-all duration-300 ease-in-out lg:translate-x-0 lg:static ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="h-full flex flex-col">
          ...
          <div className="p-6 h-16 flex items-center justify-between border-b border-white/5">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 bg-emerald-500 rounded-lg flex items-center justify-center text-[#111827] shadow-[0_0_15px_rgba(16,185,129,0.4)]">
                <span className="font-bold text-lg leading-none text-emerald-950">
                  S
                </span>
              </div>
          ...
          <div className="p-4 border-t border-white/5 bg-black/10">
            <button
              onClick={openModal}
              className="flex items-center gap-3 w-full px-4 py-3 text-slate-400 hover:bg-red-500/10 hover:text-red-400 rounded-xl transition-all font-semibold text-sm group"
            >
              <LogOut
                size={18}
