...
import { Eye, Edit3, Trash2, ArrowUpDown, Search, RefreshCw, ChevronLeft, ChevronRight, ImageIcon, Newspaper } from "lucide-react";
...
export default function BlogTable({ refreshStats }) {
    ...
    return (
        <div className="bg-white border border-slate-200 rounded-lg shadow-sm text-slate-900 overflow-hidden">
                            ...
                            paginatedData.map((b, i) => (
                                <tr key={b._id} className="hover:bg-slate-50/80 transition-colors">
                                    ...
                                    <td className="px-4 py-4 max-w-[250px]">
                                        <div className="font-bold text-slate-900 truncate" title={b.title}>{b.title}</div>
                                        <div className="text-[10px] text-slate-400 font-medium uppercase tracking-tighter truncate">{b.slug}</div>
                                    </td>

                                    <td className="px-4 py-4">
                                        <span className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded text-[11px] font-bold border border-slate-200 uppercase">{b.category}</span>
                                    </td>

                                    <td className="px-4 py-4 text-slate-600 font-medium">{b.author?.name || "Admin"}</td>

                                    <td className="px-4 py-4">
                                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${b.status === "Active" ? "bg-emerald-100 text-emerald-800" : "bg-rose-100 text-rose-800"}`}>{b.status}</span>
                                    </td>

                                    <td className="px-4 py-4 text-right">
                                        <div className="flex justify-end gap-4 text-slate-400">
                                            <button title="Preview" className="hover:text-slate-900 transition-colors" onClick={() => { setViewBlog(b); setShowDrawer(true); }}>
                                                <Eye size={18} />
                                            </button>
                                            <button title="Edit" className="hover:text-blue-600 transition-colors" onClick={() => { setEditData(b); setShowModal(true); }}>
                                                <Edit3 size={18} />
                                            </button>
                                            <button title="Delete" className="hover:text-rose-600 transition-colors" onClick={() => { setDeleteId(b._id); setShowDeleteModal(true); }}>
                                                <Trash2 size={18} />
                                            </button>
                                        </div>
                                    </td>
            ...
            <div className="p-4 border-t border-slate-100 flex flex-col md:flex-row items-center justify-between bg-slate-50/50 gap-4">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
                    ...
                    <div className="flex items-center gap-1">
                        <button disabled={currentPage === 1} onClick={() => setCurrentPage(prev => prev - 1)} className="p-2 border border-slate-200 rounded-lg bg-white disabled:opacity-30 hover:bg-slate-50 transition-colors">
                            <ChevronLeft size={16} />
                        </button>
                        <div className="flex items-center gap-1">
                        ...
                        <button disabled={currentPage >= totalPages} onClick={() => setCurrentPage(prev => prev + 1)} className="p-2 border border-slate-200 rounded-lg bg-white disabled:opacity-30 hover:bg-slate-50 transition-colors">
            ...
            <BlogModal isOpen={showModal} onClose={() => setShowModal(false)} onSuccess={fetchBlogs} initialData={editData} />
            <ViewBlogDrawer isOpen={showDrawer} onClose={() => setShowDrawer(false)} blog={viewBlog} />
