...
import { X, Tag, Link, Layers, Calendar, Activity, ImageIcon, ExternalLink } from "lucide-react";
...
export default function ViewCategoryDrawer({ isOpen, onClose, category }) {
    ...
    const imageUrl = category.image?.url 
        ? `${IMAGE_BASE_URL}/${category.image.url.replace(/\\/g, '/')}`.replace(/([^:]\/)\/+/g, "$1")
        : null;
                ...
                <div className="p-6 space-y-6 overflow-y-auto h-[calc(100%-160px)]">
                    ...
                    <div className="relative group">
                        <div className="w-full aspect-video rounded-2xl bg-slate-100 border-2 border-dashed border-slate-200 overflow-hidden flex items-center justify-center">
                            {imageUrl ? (
                                <img src={imageUrl} alt={category.name} className="w-full h-full object-contain p-2" />
                            ) : (
                                <div className="text-center text-slate-400">
                                    <ImageIcon size={40} className="mx-auto mb-2 opacity-20" />
                                    <p className="text-xs font-medium uppercase tracking-tighter">No Image Available</p>
                                </div>
                            )}
                        </div>
                        {imageUrl && (
                    ...
                    <div className="grid gap-4">
                        ...
                        <div className="flex items-center gap-4 p-4 rounded-xl border border-slate-100 bg-slate-50/50">
                            ...
                            <div>
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Parent Category</p>
                        ...
                        <div className="flex items-center gap-4 p-4 rounded-xl border border-slate-100 bg-slate-50/50">
                            ...
                            <div>
                                ...
                                <span className={`inline-block mt-1 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border ${
                        ...
                        <div className="flex items-center gap-4 p-4 rounded-xl border border-slate-100 bg-slate-50/50">
                            ...
                            <div>
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Date Added</p>
                                <p className="text-sm font-bold text-slate-900">
