...
export default function CouponModal({
  ...
  const [selectedItems, setSelectedItems] = useState([]); // Store full objects { _id, name } for display
  ...
  useEffect(() => {
    if (isOpen) {
      setFormData({
        ...
        applicableIds: [], // We will derive this from selectedItems on submit
    ...
    const payload = {
      ...
      maxDiscountAmount:
        formData.offerType === "FixedAmount"
          ? undefined // not applicable — offerValue IS the exact deduction
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200">
        ...
        >
          ...
          {formData.offerType === "BOGO" ? (
            ...
            <>
              {formData.offerType === "FixedAmount" ? (
                /* ── Fixed Amount: single field, no max discount ── */
                ...
                /* ── Percentage: value + optional max cap ── */
