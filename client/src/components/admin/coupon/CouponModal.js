...
export default function CouponModal({ isOpen, onClose, onSuccess, initialData = null }) {
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            ...
            offerValue: formData.offerType === "BOGO" ? 0 : Number(formData.offerValue),
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200">
                ...
                <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    <div className="flex items-center gap-3">
                        <div className={`p-2 ${formData.couponType === "FestiveSale" ? "bg-orange-600" : (initialData ? 'bg-blue-600' : 'bg-slate-900')} text-white rounded-lg transition-colors`}>
                            {formData.couponType === "FestiveSale" ? <Zap size={18} /> : (initialData ? <Edit3 size={18} /> : <Ticket size={18} />)}
                        </div>
                        <div>
                            <h2 className="text-lg font-bold text-slate-900 leading-tight">
                                {formData.couponType === "FestiveSale" ? "Festive Sale Configuration" : (initialData ? "Edit Coupon Details" : "Add New Coupon")}
                            </h2>
                            <p className="text-xs text-slate-500 font-medium italic">
                                {formData.couponType === "FestiveSale" ? "Auto-applied for all customers" : "Code-based promotional logic"}
                            </p>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto custom-scrollbar">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-900 uppercase tracking-wider">Campaign Type</label>
                            <select
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-900 uppercase tracking-wider">Offer Type</label>
                            <select
                    ...
                    {formData.couponType === "CouponCode" ? (
                        <div className="space-y-1.5 animate-in fade-in slide-in-from-top-1 duration-300">
                            <label className="text-xs font-bold text-slate-900 uppercase tracking-wider">Coupon Code</label>
                            <input
                                value={formData.code}
                        ...
                        <div className="p-3 bg-orange-50 border border-orange-200 rounded-lg flex items-start gap-3 animate-in fade-in slide-in-from-top-1 duration-300">
                            ...
                            <p className="text-xs text-orange-800 leading-relaxed">
                                <strong>Auto-Apply Mode:</strong> Customers don't need to enter a code. This discount will be automatically applied based on the rules below.
                            </p>
                        </div>
                    )}
                    ...
                    {formData.offerType === "BOGO" ? (
                        <div className="grid grid-cols-2 gap-4 bg-blue-50/50 p-4 rounded-xl border border-blue-100">
                            <div className="space-y-1.5">
                                <label className="text-xs font-bold text-blue-900 uppercase tracking-wider">Buy Quantity</label>
                                <input
                                    type="number"
                                    min="1"
                            ...
                            <div className="space-y-1.5">
                                <label className="text-xs font-bold text-blue-900 uppercase tracking-wider">Get Free Qty</label>
                                <input
                                    type="number"
                                    min="1"
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                                <label className="text-xs font-bold text-slate-900 uppercase tracking-wider">Discount Value {formData.offerType === 'Percentage' ? '(%)' : '(₹)'}</label>
                                <input
                                    type="number"
                            ...
                            <div className="space-y-1.5">
                                <label className="text-xs font-bold text-slate-900 uppercase tracking-wider">Max Discount (₹)</label>
                                <input
                                    type="number"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-900 uppercase tracking-wider">Min Order (₹)</label>
                            <input
                                type="number"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-900 uppercase tracking-wider">Total Usage Limit</label>
                            <input
                                type="number"
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-900 uppercase tracking-wider">Limit Per User</label>
                            <input
                                type="number"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-900 uppercase tracking-wider">End Date</label>
                            <input
                                type="date"
                    ...
                    {/* Description */}
                    <div className="space-y-1.5">
                        <label className="text-xs font-bold text-slate-900 uppercase tracking-wider">Sale/Coupon Description</label>
                        <textarea
                            rows="2"
                    ...
                    <div className="pt-4 flex items-center gap-3">
                        ...
                        >
                            {loading ? <Loader2 className="animate-spin" size={18} /> : (initialData ? "Update Campaign" : "Launch Campaign")}
                        </button>
                    </div>
                </form>
            </div>
