...
import {
    ...
    MoreHorizontal, ArrowRight, Package,
...
export default function OrdersPage() {
    ...
    // Dummy Orders Data
    const orders = [
        { id: "#ORD-7721", customer: "Rahul Verma", date: "28 Dec, 2025", total: "₹2,499", status: "Delivered", method: "UPI" },
        { id: "#ORD-7722", customer: "Sneha Kapoor", date: "27 Dec, 2025", total: "₹1,250", status: "Processing", method: "COD" },
        { id: "#ORD-7723", customer: "Amit Singh", date: "27 Dec, 2025", total: "₹8,990", status: "Shipped", method: "Card" },
        { id: "#ORD-7724", customer: "Priya Das", date: "26 Dec, 2025", total: "₹3,200", status: "Pending", method: "UPI" },
    ];
        ...
        <div className="w-full animate-in fade-in duration-500 pb-10">
            ...
            <div className="bg-white rounded-[28px] border border-slate-200 shadow-sm overflow-hidden">
                            ...
                            {orders.map((order, i) => (
                                <tr key={i} className="hover:bg-slate-50/50 transition-colors group">
                                    <td className="px-6 py-5">
                                        ...
                                        <p className="text-[10px] font-bold text-slate-400 uppercase mt-0.5">{order.method}</p>
                                    </td>
                                    <td className="px-6 py-5">
                                    ...
                                    <td className="px-6 py-5">
                                        <span className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase border ${getStatusStyle(order.status)}`}>
                                            {order.status}
                                        </span>
                                    </td>
                                    <td className="px-6 py-5">
                                        <p className="text-sm font-black text-slate-900 font-mono">{order.total}</p>
                                    </td>
                ...
                <div className="p-6 border-t border-slate-50 flex justify-between items-center bg-slate-50/30">
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Showing 4 of 120 orders</p>
                    <div className="flex gap-2">
                        <button className="px-4 py-2 bg-white border border-slate-200 rounded-lg text-[10px] font-black uppercase text-slate-400 cursor-not-allowed">Prev</button>
                        <button className="px-4 py-2 bg-white border border-slate-200 rounded-lg text-[10px] font-black uppercase text-slate-900 hover:bg-slate-50">Next</button>
                    </div>
                </div>
            </div>
        </div>
