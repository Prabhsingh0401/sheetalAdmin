...
export default function TextPagesManager() {
  ...
  const pages = [
    ...
    },
    {
      ...
      description: "Usage rules and legal agreements",
    ...
    },
    {
      ...
      description: "Data protection and cookies policy",
      slug: "privacy-policy",
    ...
    <div className="min-h-screen w-full animate-in fade-in duration-500 pb-10">
      ...
      <div className="space-y-4">
        {pages.map((page) => (
          <div
            key={page.id}
            className="bg-white p-5 border border-slate-200 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between hover:border-slate-900 transition-all group shadow-sm"
          >
            <div className="flex items-center gap-5">
              <div
                className={`p-4 rounded-xl shadow-sm ${page.color === "blue" ? "bg-blue-50 text-blue-600" : page.color === "amber" ? "bg-amber-50 text-amber-600" : "bg-emerald-50 text-emerald-600"}`}
              >
                {page.icon}
              </div>
              <div>
