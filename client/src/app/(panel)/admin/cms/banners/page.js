    ...
    return (
        <div className="min-h-screen w-full animate-in fade-in duration-500 pb-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8 mt-8">
                ...
                <StatCard title="Active" count={stats.active} icon={<CheckCircle2 size={20} />} color="emerald" />
                <StatCard title="Inactive" count={stats.inactive} icon={<XCircle size={20} />} color="rose" />
                <StatCard title="Live Status" count={stats.active > 0 ? "Live" : "No"} icon={<Monitor size={20} />} color="amber" />
            </div>
            ...
            {isModalOpen && (
                <BannerModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSuccess={fetchStats} />
            )}
        </div>
    );
}
