    ...
    return (
        <div className="min-h-screen w-full animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
                <StatCard title="Total" count={stats.total} icon={<Ticket size={18} />} color="indigo" />
                <StatCard title="Active Now" count={stats.active} icon={<Zap size={18} />} color="emerald" />
                <StatCard title="Festive/Auto" count={stats.festiveSales} icon={<Percent size={18} />} color="rose" />
                <StatCard title="Redemptions" count={stats.totalUsed} icon={<Tag size={18} />} color="slate" />
                <StatCard title="Savings" count={stats.totalSavings} icon={<Gift size={18} />} color="amber" isCurrency={true} />
            </div>
...
function StatCard({ title, count, icon, color, isCurrency = false }) {
    ...
    return (
        <div className="bg-white p-5 border border-slate-200 rounded-2xl flex items-center gap-4 hover:shadow-lg hover:border-slate-300 transition-all duration-300 group">
            <div className={`p-3 rounded-xl border ${colors[color]} group-hover:scale-110 transition-transform duration-300`}>{icon}</div>
            <div>
                ...
                <p className="text-xl font-extrabold text-slate-800 mt-2 leading-none">
                    {isCurrency ? `₹${safeCount.toLocaleString()}` : safeCount.toLocaleString()}
                </p>
            </div>
        </div>
    );
