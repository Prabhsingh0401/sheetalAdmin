...
export const createProductReview = async (req, res, next) => {
    try {
        ...
        const result = await productService.addReviewService(productId, req.user, rating, comment);
        if (!result.success) return res.status(result.statusCode).json(result);
...
export const bulkImportProducts = async (req, res, next) => {
    try {
        ...
        const result = await productService.bulkImportService(req.file.path, req.user._id);
        if (fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path);

        return successResponse(res, 200, result.data, "Bulk products imported successfully");
    } catch (error) {
        ...
        if (req.files) {
            ...
            for (const file of allFiles) {
                ...
                if (file.size > limit) {
                    ...
                    return next(ErrorResponse(`${file.fieldname} (${file.originalname}) size exceeds the ${isVideo ? '50MB' : '5MB'} limit.`, 400));
                }
            }
        }

        const result = await productService.createProductService(req.body, req.files, req.user._id);
        if (!result.success) {
            clearFiles(req.files);
...
export const updateProduct = async (req, res, next) => {
    try {
        const result = await productService.updateProductService(req.params.id, req.body, req.files);
        if (!result.success) {
            clearFiles(req.files);
            return res.status(result.statusCode).json(result);
        }
...
export const deleteReview = async (req, res, next) => {
    try {
        const result = await productService.deleteReviewService(req.query.productId, req.query.id);
        if (!result.success) return res.status(result.statusCode).json(result);
